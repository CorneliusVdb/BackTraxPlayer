...
Public Sub UpdateSpectrum(ByVal uTimerID As Long, ByVal uMsg As Long, ByVal dwUser As Long, ByVal dw1 As Long, ByVal dw2 As Long, Channel As Integer)
    ...
    If (specmode = 3) Then ' waveform
        ...
        Call BASS_ChannelGetInfo(chan(Channel), ci) ' get number of channels
        ReDim buf(ci.chans * SPECWIDTH * LenB(buf(0))) As Single  ' allocate buffer for data

        Call BASS_ChannelGetData(chan(Channel), buf(0), (ci.chans * SPECWIDTH * LenB(buf(0))) Or BASS_DATA_FLOAT)  ' get the sample data (floating-point to avoid 8 & 16 bit processing)
    ...
    Else
        ...
        Call BASS_ChannelGetData(chan(Channel), fft(0), BASS_DATA_FFT2048)
...
#End If
    ...
    Call SetDIBitsToDevice(frmPlayer.Picture1.hDC, 0, 0, SPECWIDTH, SPECHEIGHT / 4, 0, 0, 0, SPECHEIGHT, specbuf(0), bh, 0)
End Sub
