Attribute VB_Name = "modGridModule"
...
Public Sub FlexGridScroll(ByRef FG As MSFlexGrid, ByVal MouseKeys As Long, ByVal Rotation As Long, ByVal Xpos As Long, ByVal Ypos As Long)
  ...
  On Error Resume Next
  With FG
    Lstep = .Height / .RowHeight(1) 'Have to use row 1, since we made row(0).height = 0
    Lstep = Int(Lstep)
    If .Rows < Lstep Then Exit Sub
    Lstep = Lstep / 7  'We have 14 visible rows, so devide by 7 to scroll 2 rows
...
Public Sub HorFlexGridScroll(ByRef HFG As MSFlexGrid, ByVal MouseKeys As Long, ByVal Rotation As Long, ByVal Xpos As Long, ByVal Ypos As Long)
  Dim NewValue As Long
  Dim Lstep As Single
